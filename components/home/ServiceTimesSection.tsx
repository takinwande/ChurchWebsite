...
export function ServiceTimesSection({ serviceTimes }: ServiceTimesSectionProps) {
  ...
  return (
    <FadeIn>
    <section className="bg-primary py-12 text-primary-foreground" aria-label="Service times">
        ...
        <div className="flex flex-col items-center gap-6 sm:flex-row sm:flex-wrap sm:justify-center sm:gap-x-10 sm:gap-y-6 lg:gap-x-16">
          ...
          {times.map((s, i) => (
            <div key={i} className="text-center">
              <p className="text-xs font-semibold uppercase tracking-wider text-primary-foreground/70">{s.name}</p>
